...
public class Utilities {
    ...
    public static void outputToJavaFile(String profileName, Trajectory tr) throws FileNotFoundException {
        ...
        for(int i = 0; i < leftTraj.length(); i++) {
            ...
            leftWriter.append("{"+(leftTraj.segments[i].position / (Math.PI * wheelbaseDiameter)) + ", " +

                    (leftTraj.segments[i].velocity * 60 / (Math.PI * wheelbaseDiameter)) + ", " +

                    (leftTraj.segments[i].dt * 1000)+ "} ");

            rightWriter.append("{"+(rightTraj.segments[i].position / (Math.PI * wheelbaseDiameter)) + ", " +

                    (rightTraj.segments[i].velocity * 60 / (Math.PI * wheelbaseDiameter)) + ", " +
